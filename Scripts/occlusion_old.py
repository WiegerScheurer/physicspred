...
def surprise_trial(verbose: bool = True):
    global surprisal_idx
    ratio = surprise_dict[surprisal_modes[surprisal_idx]]
    ...
    if verbose:
        print(
            f"Surprisal mode: {surprisal_modes[surprisal_idx]}, this trial: {ball_behaviour}"
        )
...
def spawn_ball_around_square(
    ...
    while not desirable_angle:
        ...
        s = np.max(int_loc) / 2 + spawn_margin
...
def reset_trial(verbose: bool = True):
    ...
    global ball_angle, draw_simulation
...
while running:
    ...
    if current_time - trial_start < 2000:
        ...
        if ball_behaviour == "realistic":
            ...
            if interactor_type == "line":
                ...
                bounced_yet = True
                    # Now commented out, because I wanted to make a distinction between oblique and orthogonal bounces, but
                    #  they are all supposed to be oblique/ diagnoal now, so does the same

                    # ball_velocity = bounce_back(
                    #     interactor_angle,
                    #     ball_velocity,
                    #     ball_pos,
                    #     screen_size,
            ...
            if interactor_type == "curve":
                ...
                curve_center = [screen_size // 2, screen_size // 2]
                if (
                    math.dist(ball_pos, curve_center)
                    < INTERACTOR_SIZE + occluder_excess
                ):
                    if arc_angle == 0 and (
                    ...
                    elif arc_angle == 3 and (direction == "left" or direction == "top"):
                        ball_velocity = bounce_back(
                            interactor_angle,
                            ball_velocity,
                            ball_pos,
                    ...
                    elif math.dist(ball_pos, curve_center) < INTERACTOR_SIZE / 2:
                        inner_bounce_angle = arc_angle % 2 * 90 + 45
                        ball_velocity = diag_bounce(
                            inner_bounce_angle,
                            ball_pos,
                            ball_velocity,
            ...
            if (
                ...
                if stochastic_type == "omission":
                    ...
                    # phantom_interactor_angle = phantom_bounce_angle
    ...
    if current_time - trial_start > 5000:
        ball_visible = reset_trial()
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_f:  # Make ball faster
                BALL_SPEED += 10 if BALL_SPEED < 100 else -90
                print(f"Changing speed to {BALL_SPEED}")
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_b:  # Make ball bigger
                BALL_RADIUS += 10 if BALL_RADIUS < 100 else -90
