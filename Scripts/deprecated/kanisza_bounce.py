...
def draw_kanisza(screen, screen_size, pacman_radius=130, offset_factor=2.5, num_corners=3, partial_shape=True, tilt=1):
    ...
    positions = [
        (
            center_x + offset * math.cos(math.radians(angle_step * i + 45*tilt)),
            center_y + offset * math.sin(math.radians(angle_step * i + 45*tilt))
        )
...
def draw_interactor(kanisza_input=None):
    ...
    if interactor_type == "line":
        ...
        pygame.draw.line(screen, WHITE, (x1, y1), (x2, y2), 16)
...
def draw_occluder(draw_it:bool=False):
...
def reset_trial(verbose: bool = True):
    ...
    global ball_colour, delay_time, n_superimposed_corners, this_tilt
    ...
    n_superimposed_corners = 4
    this_tilt = random.choice([1, 3, 5, 7])
...
# def check_collision(angle, ball_pos, screen_size, INTERACTOR_SIZE, line: bool):
#     rad_angle = math.radians(angle)
#     line_dx = INTERACTOR_SIZE * math.cos(rad_angle)
#     line_dy = INTERACTOR_SIZE * math.sin(rad_angle)
...
def check_collision(angle, ball_pos, screen_size, INTERACTOR_SIZE, line: bool):
    ...
    if not bounced_yet:
        ...
        # Check if the ball touches the region
        if (
            x_min - BALL_RADIUS <= ball_pos[0] <= x_max + BALL_RADIUS
            and y_min - BALL_RADIUS <= ball_pos[1] <= y_max + BALL_RADIUS
        ):
            bounced_yet = True
            return True
    return False

def calculate_bounce_velocity(angle, ball_velocity):
    rad_angle = math.radians(angle)
    normal_angle = rad_angle + math.pi / 2
    normal = [math.cos(normal_angle), math.sin(normal_angle)]
...
def get_hypothetical_velocity(ball_angle):
    ...
    hypothetical_velocity = calculate_bounce_velocity(angle=interactor_angle, ball_velocity=original_velocity)
...
while running:
    ...
    if superimpose_triangle:
        draw_kanisza(screen, screen_size, num_corners=n_superimposed_corners, tilt=this_tilt)
    ...
    if current_time - trial_start < 2000:
        ...
        if ball_behaviour == "realistic":
            ...
            # Line interaction
            if interactor_type == "line" and check_collision(angle=interactor_angle, 
                                                             ball_pos=ball_pos, 
                                                             screen_size=screen_size, 
                                                             INTERACTOR_SIZE=INTERACTOR_SIZE, 
                                                             line=True):
                
                print("new function works ")
                if interactor_angle % 90 != 0:
                    
                    ball_velocity = calculate_bounce_velocity(angle=interactor_angle,
            ...
            if (
                ...
                if (
                    ...
                    if stochastic_type == "omission":
                        ...
                        if ball_behaviour == "no_interactor" and check_collision(angle=interactor_angle, 
                                                                                 ball_pos=ball_pos, 
                                                                                 screen_size=screen_size, 
                                                                                 INTERACTOR_SIZE=INTERACTOR_SIZE, 
                                                                                 line=True): # Not sure if necessary
                            ...
                            hypothetical_ball_velocity = calculate_bounce_velocity(angle=interactor_angle,
                                                                      ball_velocity=ball_velocity)
