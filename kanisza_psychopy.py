...
# Define stimuli
fixation = visual.TextStim(win, text='+', color='grey', pos=(0, 0), height=40)
...
circle_radius = 100
circle_spread = 200
circle_positions = [(-circle_spread, circle_spread), (circle_spread, circle_spread), (-circle_spread, -circle_spread), (circle_spread, -circle_spread)]

def create_pacman(win, radius, start_angle, end_angle, pos):
    vertices = [(0, 0)]
    for angle in np.linspace(start_angle, end_angle, 50):
        x = radius * np.cos(np.radians(angle))
        y = radius * np.sin(np.radians(angle))
        vertices.append((x, y))
    return visual.ShapeStim(win, vertices=vertices, fillColor='white', lineColor='white', pos=pos)

triangle_configs = {
    "top_left": [(0, 270), (225, 540), (90, 405), (180, 450)],
    "top_right": [(0, 315), (270, 540), (270, 585), (135, 450)],
    "bottom_left": [(315, 630), (90, 360), (90, 360), (180, 495)],
    "bottom_right": [(180, 450), (270, 585), (45, 360), (180, 450)]
}

def configure_circles(kanizsa, verbose=False):
    circles = []
    if kanizsa:
        which_triangle = random.choice(list(triangle_configs.keys()))   # Randomly select a triangle configuration
        print(f"Selected triangle: {which_triangle}") if verbose else None
        # Arrange circles to form a Kanizsa triangle
        circles.append(create_pacman(win, circle_radius, triangle_configs[which_triangle][0][0], triangle_configs[which_triangle][0][1], circle_positions[0]))  # Top-left
        circles.append(create_pacman(win, circle_radius, triangle_configs[which_triangle][1][0], triangle_configs[which_triangle][1][1], circle_positions[1]))  # Top-right
        circles.append(create_pacman(win, circle_radius, triangle_configs[which_triangle][2][0], triangle_configs[which_triangle][2][1], circle_positions[2]))  # Bottom-left
        circles.append(create_pacman(win, circle_radius, triangle_configs[which_triangle][3][0], triangle_configs[which_triangle][3][1], circle_positions[3]))  # Bottom-right
    else:
        # Arrange circles in a random configuration
        for pos in circle_positions:
            which_triangle = random.choice(list(triangle_configs.keys())) # Randomly select a triangle configuration
            start_angle, end_angle = triangle_configs[which_triangle][random.randint(0, 3)]
            circles.append(create_pacman(win, circle_radius, start_angle, end_angle, pos))
    return circles, which_triangle

def update_start_positions(triangle_config_name, win_dims):
    ...
    # Extract directions from the triangle configuration name
    directions = triangle_config_name.split('_')
    
    # Remove the directions from the start positions
    updated_start_positions = {k: v for k, v in start_positions.items() if k not in directions}
    
    return updated_start_positions
...
for trial in trial_conditions:
    ...
    kanizsa = random.choice([True, False])
    
    circles, this_triangle_config = configure_circles(kanizsa, verbose=True)
    
    if kanizsa:
        trial = "45" if this_triangle_config in ["top_left", "bottom_right"] else "135"
    else:
        trial = "none"
    ...
    if trial == "45":
        ...
        # line_135.draw()
        pass
    print(f"Kanizsa triangle: {kanizsa}") #if verbose else None
    configure_circles(kanizsa)
    ...
    updated_start_positions = update_start_positions(this_triangle_config, win_dims)
    edge = random.choice(list(updated_start_positions.keys()))
    ball.pos = np.array(updated_start_positions[edge])
